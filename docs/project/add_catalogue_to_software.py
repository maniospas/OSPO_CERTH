...
out_file = Path(output_dir/"docs/project/software-en.html")
...
out_file = Path(output_dir/"docs/project/software-gr.html")
catalogue = template.replace("{{CONTENTS}}", text)
html = out_file.read_text(encoding="utf-8")
html = html.replace("</body>", catalogue + "\n</body>", 1)
html = html.replace("Search projects", "Αναζήτηση λογισμικού")
html = html.replace("Showing ${visible.length} result${visible.length > 1 ? 's' : ''}", "Εμφάνιση ${visible.length} ${visible.length > 1 ? 'αποτελεσμάτων' : 'αποτελέσματος'}")
html = html.replace('No results found', "Κανένα αποτέλεσμα")
out_file.write_text(html, encoding="utf-8")
